...
class WebsocketReceiver(Receiver):
    ...
    def __init__(self, cert: Cert, compress: bool, baseUrl: str):
        super().__init__()
        self._cert = cert
    ...
    async def _get_gateway(self, cs: ClientSession):
        ...
        async with cs.get(
            f"{self.baseUrl}/gateway/index", headers=headers, params=params
        ) as res:
...
class WebhookReceiver(Receiver):
    ...
    async def start(self):
        ...
        async def on_recv(request: web.Request):
            ...
            if (
                pkg["d"]["verify_token"] != self._cert.verify_token
            ):  # check verify_token
                return web.Response()
